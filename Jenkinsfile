      ...
      steps {
        ...
        withAWS(credentials: 'JenkinsBEN', region: AWS_REGION) {
          script {
            ...
            sh "aws elasticbeanstalk update-environment --environment-name ${env.ELASTIC_BEANSTALK_ENVIRONMENT} --version-label ${env.BUILD_NUMBER} --option-settings Namespace=aws:elasticbeanstalk:environment,OptionName=PROC,Value='npm start'"
          }
        }
      }
    }
